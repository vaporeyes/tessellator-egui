...
impl Painter {
    ...
    async fn add_surface(
        ...
        // Patched (tessellator local fork): on macOS the window's inner_size
        // is briefly reported as 0xN at this point in startup, which causes
        // configure_surface to be skipped and the next paint to panic with
        // "Surface is not configured for presentation". We still insert the
        // SurfaceState so on_window_resized can find it later, but flag
        // needs_reconfigure so the paint path will configure once a real
        // size arrives. paint_and_update_textures additionally short-circuits
        // when width/height are zero (see below).
        let zero_size = size.width == 0 || size.height == 0;
        ...
        #[cfg(all(target_os = "macos", feature = "macos-window-resize-jitter-fix"))]
        {
            ...
            unsafe {
                if let Some(hal_surface) = state.surface.as_hal::<wgpu::hal::api::Metal>() {
                    ...
                    // Patched (tessellator local fork): skip the
                    // jitter-fix configure when the surface still has zero
                    // size. wgpu validates non-zero on Surface::configure
                    // and panics otherwise; see the on_window_resized path
                    // for the equivalent NonZero gate.
    ...
    pub fn paint_and_update_textures(
        ...
        // Patched (tessellator local fork): bail out of the present path if
        // the surface still has zero size. We've already updated egui's
        // texture state above (which is required to stay consistent), but
        // wgpu validates non-zero on Surface::configure and on
        // Surface::get_current_texture, so attempting either here would
        // panic. on_window_resized will configure us properly once the OS
        // reports the real window size and the next frame will render.
        if surface_state.width == 0 || surface_state.height == 0 {
            // Submit any command-buffer work the renderer has queued
            // (write_buffer etc) so internal staging buffers don't leak.
            render_queue_guard.commands_submitted = true;
            render_state.queue.submit(
                user_cmd_bufs.into_iter().chain(std::iter::once(encoder.finish())),
            );
            return vsync_sec;
        }
